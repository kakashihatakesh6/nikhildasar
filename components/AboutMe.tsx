...
const AboutMe = () => {
    return (
        <div className="mt-12 md:mt-20 h-auto w-full flex flex-col" id="about">
            <div 
                className="stroke-yellow-300 text-[1.8rem] xs:text-[2.2rem] sm:text-[3.2rem] md:text-[5rem] lg:text-[6rem] font-extrabold z-0 opacity-80 text-transparent select-none leading-none mb-6 whitespace-nowrap" 
            ...
            <div className="w-full mt-6 bg-slate-900/20 backdrop-blur-md border border-slate-800/80 rounded-3xl p-5 md:p-8 shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden" >
                ...
                <Tabs defaultValue="skills" className="w-full">
                    ...
                    <TabsList className="flex bg-slate-950/60 p-1 rounded-2xl border border-slate-800/80 w-full mb-8 h-auto gap-1 text-slate-400">
                        <TabsTrigger 
                            value="skills" 
                            className="flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-3 text-[10px] sm:text-xs md:text-sm font-bold uppercase tracking-wider rounded-xl transition-all duration-300 data-[state=active]:bg-yellow-400 data-[state=active]:text-black text-slate-400 hover:text-slate-200"
                        >
                            <Brain className="w-4 h-4 hidden sm:inline-block" />
                            <span>Skills</span>
                        </TabsTrigger>
                        <TabsTrigger 
                            value="education" 
                        ...
                        <TabsTrigger 
                            ...
                            className="flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-3 text-[10px] sm:text-xs md:text-sm font-bold uppercase tracking-wider rounded-xl transition-all duration-300 data-[state=active]:bg-yellow-400 data-[state=active]:text-black text-slate-400 hover:text-slate-200"
                        >
                            <FileText className="w-4 h-4 hidden sm:inline-block" />
                            <span>Resume</span>
                        </TabsTrigger>
                    </TabsList>
                    ...
                    <TabsContent value="skills" className="outline-none">
                        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3 w-full m-auto justify-items-center pt-2">
                            <Skills />
                        </div>
                    </TabsContent>
